...
{
public:
    void ReadREEL(QDataStream &in);
    void WriteREEL(QDataStream &out);
...
public:/*The members*/
    ...
    qint64 TotalTraces;/*total traces*/
    int DataType;/*How many bytes per every number*/
...
{
public:/*Functions*/
    void ReadTRACE(QDataStream &in);
    void WriteTRACE(QDataStream &out);
public:/*Members*/
    ...
    short unass[14];
                    /* unassigned--NOTE: last entry causes
...
unsigned int num2ibm(float input);/*Write IBM*/
float ibm2num(unsigned int DataUint32);/*Read IBM*/
float ieee2num(unsigned int ieee);
void ReadDataATP(QDataStream &in,float x[],int N,int flags);/*读取所有格式数据的函数,flags 代表数据格式码*/
void WriteDataATP(QDataStream &out,float x[],int N,int flags);/*读取所有格式数据的函数,flags 代表数据格式码*/
