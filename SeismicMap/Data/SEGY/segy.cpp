...
float ibm2float(const float data)
{
...
int float2ibm(const float data)
{
    register int fconv, fmant, t;
    fconv = *((int*)(&data));
    if (fconv)
...
{
    mseek64(file,0L,SEEK_END);
    long long pos = mtellg64(file);
...
long long mseek64(FILE *file, long long offset, int whence)
{
#ifdef WIN32
    return  _lseeki64(fileno(file),offset,whence);
#else
    return lseek(fileno(file),offset,whence);
#endif
}
long long mtellg64(FILE *file)
{
#ifdef WIN32
    return _telli64(fileno(file));
#else
    return ftell(file);
#endif
}
...
void ReadData(FILE *file, float *data, SEGYHEAD *segyhead)
{
    int N = segyhead->hns;
    switch( segyhead->format )
    {
    case 0:
    ...
    case 3:
    {
        ...
        for( i = 0; i < N; i++ )
        {
            ...
            //printf("%f\t%d\n",data[i],swap_int16(temp[i]));
            //qDebug()<<data[i]<<swap_int16(temp[i]);
        }
        free(temp);
        break;
    }
...
void WriteData(FILE *file, float *data, SEGYHEAD *segyhead)
{
    int N = segyhead->hns;
    switch(segyhead->format)
    {
